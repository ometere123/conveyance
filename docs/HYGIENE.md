...
| Contract addresses | 5 distinct | **HISTORICAL / SUPERSEDED at the time this sweep was written; none of the five is current.** `0x104767ad5d51b5004953e4fB9d5B548501aa9bd9` was the live deployment when this sweep ran, and is superseded now: the canonical address is `0x7C2f0B5F397957214b7D15120dCb9A5cDbd282d1` (see `DEPLOYMENT.json` and `.env.local`/`.env.example`, which name the current address only). `0x104767ad...` is not yet carried in `DEPLOYMENT.json`'s `historicalDeployments` array; `evidence/studionet.json` and `README.md` already label it historical. Of the other four: `0xC162BA113137539fD734e986403C1FA5fAbA6109` carried the seller casing defect, `0x07a615f2F2D7fC48AF2630b410e403a82B1d261b` refused invalid input by raising after value had already arrived, `0xB869533BEE20269514c7552dc85215e10d8b7A75` had a defect in the per-domain deal index, and `0x513B913dfbCB1790a35630f100CB59140a67B4ba` is listed separately because it reported ACCEPTED and then FINALIZED and returned an address while registering nothing, which is what a missing GenVM runtime pin does. No live claim in this repository rests on any of the five. |
